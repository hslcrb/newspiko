...
def test_analyzer_prompt_generation():
    ...
    with patch('groq.Groq') as mock_groq:
        ...
        analyzer.client.chat.completions.create.return_value.choices[0].message.content = "분석 완료 [KEYWORDS: [\"A\"]] [POLITICAL_SENTIMENT: sl=100, ml=0, mr=0, sr=0] [SUSPICION: 0]"
...
def test_sentiment_parsing_in_main():
    ...
    analyzer = MagicMock()
    analyzer.analyze_opinion.return_value = "결과 [POLITICAL_SENTIMENT: sl=70, ml=0, mr=20, sr=10]"
    # parse_results 모킹 필수
    analyzer.parse_results.return_value = {
        "sentiment": {"sl": 70, "ml": 0, "mr": 20, "sr": 10},
        "keywords": ["A", "B"],
        "suspicion": 30
    }
    ...
    assert res_dict["sentiment"]["sl"] == 70
    assert res_dict["sentiment"]["mr"] == 20
    assert res_dict["sentiment"]["sr"] == 10
    # 원문에서 태그가 제거되었는지 확인
    assert "[POLITICAL_SENTIMENT:" not in res_dict["text"]
